...
export const calculateExpirationDate = (expiresIn: string = '15m'): Date => {
  ...
  switch (unit) {
    case 'm': // minutes
      return add(expirationDate, { minutes: value ? parseInt(value) : 0 });
    case 'h': // hours
      return add(expirationDate, { hours: value ? parseInt(value) : 0 });
    case 'd': // days
      return add(expirationDate, { days: value ? parseInt(value) : 0 });
    default:
